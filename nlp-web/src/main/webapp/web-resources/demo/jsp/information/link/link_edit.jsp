...
<body>
	<form action="link/${msg }.do" name="Form" id="Form" method="post" enctype="multipart/form-data">
		...
		<table id="table_report" class="table table-striped table-bordered table-hover">
			...
			<tr>
				<td>网名:</td>
				<td><input type="text"style="width:95%;" name="stiename" id="stiename" value="${pd.stiename }" placeholder="这里输入网站名称" title="网站名称"/></td>
			</tr>
			<tr>
				<td>内容:</td>
				<td id="nr">
					<textarea  style="width:90%;height:200px" name="sitecontent" id="content">${pd.sitecontent }</textarea>
				</td>
			</tr>
			<tr>
				<td>链接:</td>
				<td><input type="text"style="width:95%;" name="tourl" id="tourl" value="${pd.tourl }" placeholder="这里输入链接" title="链接"/></td>
			</tr>
			<tr>
				<td>图片:</td>
				<td>
			...
			<tr>
				...
				<td>
					...
					<option value="0" <c:if test="${pd.type == '0' }">selected</c:if> >文字</option>
					<option value="1" <c:if test="${pd.type == '1' }">selected</c:if> >图片</option>
					</select>
