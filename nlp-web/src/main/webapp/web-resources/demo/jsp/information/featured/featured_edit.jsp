	...
	<form action="featured/${msg }.do" name="Form" id="Form" method="post">
		...
		<table id="table_report" class="table table-striped table-bordered table-hover">
			...
			<tr>
				<td>链接:</td>
				<td>
					<input type="text" name="url" style="width:85%;" id="url" value="${pd.url }" placeholder="这里输入链接" title="链接" />
				</td>
			</tr>
			<tr>
				<td>选择:</td>
			...
			<tr>
				...
				<td>
					...
					<option value="1" <c:if test="${pd.heat == '1' }">selected</c:if> >1</option>
					<option value="2" <c:if test="${pd.heat == '2' }">selected</c:if> >2</option>
					<option value="3" <c:if test="${pd.heat == '3' }">selected</c:if> >3</option>
					<option value="4" <c:if test="${pd.heat == '4' }">selected</c:if> >4</option>
					<option value="5" <c:if test="${pd.heat == '5' }">selected</c:if> >5</option>
					</select>
					星级：
					<select name="stars" title="星级">
					<option value="1" <c:if test="${pd.stars == '1' }">selected</c:if> >1</option>
					<option value="2" <c:if test="${pd.stars == '2' }">selected</c:if> >2</option>
					<option value="3" <c:if test="${pd.stars == '3' }">selected</c:if> >3</option>
					<option value="4" <c:if test="${pd.stars == '4' }">selected</c:if> >4</option>
					<option value="5" <c:if test="${pd.stars == '5' }">selected</c:if> >5</option>
					</select>
				</td>
			</tr>
