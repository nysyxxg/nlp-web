...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="featured/list.do?isM1=${pdm.isM1 }&isM2=${pdm.isM2 }" method="post" name="userForm" id="userForm">
			<table border='0'>
				<tr>
					...
					<td><input class="span10 date-picker" name="lastStart" value="${pd.lastStart}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:100px;" placeholder="发布开始日期"/></td>
					<td><input class="span10 date-picker" name="lastEnd" value="${pd.lastEnd}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:100px;" placeholder="发布截止日期"/></td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function edit(id){
			window.parent.jzts();
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = 'featured/goEdit.do?id='+id;
			 diag.Width = 800;
			 diag.Height = 630;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function del(id){
			bootbox.confirm("确定要删除该记录?", function(result) {
				if(result) {
					var url = "featured/delete.do?id="+id+"&tm="+new Date().getTime();
